...
public class JPARegistryStorage implements RegistryStorage {
    ...
    private MetaData getMetaData(String artifactId, long version, String key) {
        return entityManager.createQuery(
            "SELECT m FROM MetaData m " +
    ...
    private Artifact _getArtifact(String artifactId, long version, EnumSet<ArtifactState> states) {
        requireNonNull(artifactId);
        try {
            MetaData md = getMetaData(artifactId, version, MetaDataKeys.STATE);
            ArtifactState state = ArtifactState.valueOf(md.getValue());
    ...
    private Artifact _getArtifact(String artifactId, EnumSet<ArtifactState> states) {
        ...
        for (Artifact artifact : artifacts) {
            MetaData md = getMetaData(artifactId, artifact.getVersion(), MetaDataKeys.STATE);
            ArtifactState state = ArtifactState.valueOf(md.getValue());
    ...
    private void _ensureArtifactExists(String artifactId, EnumSet<ArtifactState> states) {
        _getArtifact(artifactId, states);
    }
    ...
    private Artifact _getArtifact(long id, EnumSet<ArtifactState> states) {
        ...
        MetaData md = getMetaData(artifact.getArtifactId(), artifact.getVersion(), MetaDataKeys.STATE);
        ArtifactState state = ArtifactState.valueOf(md.getValue());
